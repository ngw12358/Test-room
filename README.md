...
**Swap 영역은 디스크이기 때문에 RAM 영역보다 속도가 느리다.**
...
***
# vim 에디터에서 매크로를 사용하는 방법

**매크로 기록**

vim 에디터에서 매크로를 다음과 같이 기록할 수 있다.

`q<문자><명령어>q`

>vim 노말모드에서 q를 입력하고 매크로의 이름으로 사용할 문자를 한 글자 입력하면 매크로를 기록하기 시작한다.
>
>매크로로 사용할 명령어들을 다 입력한 뒤 q를 입력하면 매크로의 기록이 완료된다.

`ex) q a kO q //윗 라인으로 이동한 후(k) 현재 라인을 다음 줄로 밀고 입력 모드를 시작(O)하는 매크로 a 기록`

***
**매크로 실행**

기록한 매크로를 다음과 같이 실행할 수 있다.
...
>vim 노말모드에서 @를 입력하고 매크로의 이름으로 등록한 문자을 누르면 등록한 매크로가 실행된다.
>
>@@를 입력하면 가장 최근에 재생한 매크로를 실행한다.

`ex) @a @@: 매크로 a 재생 후 가장 최근에 재생한 매크로 a 재생`
***
**매크로 반복 실행**

기록한 매크로를 다음과 같이 반복 실행할 수 있다.
...
>매크로를 반복할 횟수를 입력하고 매크로를 실행하면 입력한 숫자만큼 매크로를 반복실행한다.

`ex) 3@a 4@@ : 매크로 a를 3번 반복실행하고 가장 최근에 재생한 매크로 a를 4번 반복 실행`
